...
import { findWhoisServer, findRdapServer, queryIanaWhoisServer, getRdapBootstrap, extractTld, KNOWN_WHOIS_SERVERS, } from './iana.js';
const server = new McpServer({
    name: 'domain-whois-mcp',
    version: '1.0.0',
    description: 'Domain name intelligence via native WHOIS and RDAP protocols with IANA TLD discovery',
});
// ─── Tool: whois_lookup ──────────────────────────────────────────────────────
server.tool('whois_lookup', `Look up WHOIS information for a domain name using native WHOIS protocol (TCP port 43).
...
Returns parsed structured data + raw WHOIS text.`, {
    ...
    try {
        ...
        return {
            content: [{ type: 'text', text: `WHOIS lookup failed: ${message}` }],
            isError: true,
        };
    }
});
// ─── Tool: rdap_lookup ───────────────────────────────────────────────────────
server.tool('rdap_lookup', `Look up RDAP (Registration Data Access Protocol) information for a domain.
...
Not all TLDs support RDAP — falls back with a clear error message.`, {
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: 'text', text: `RDAP lookup failed: ${message}` }],
            isError: true,
        };
    }
});
// ─── Tool: domain_check ──────────────────────────────────────────────────────
server.tool('domain_check', `Check if a domain name is available for registration.
...
Returns availability status and full registration data if registered.`, {
    ...
    try {
        ...
        if (result.rdap && result.rdap.status.length > 0) {
            ...
            if (regEvent)
                text += `- **Registered:** ${regEvent.eventDate}\n`;
            if (expEvent)
                text += `- **Expires:** ${expEvent.eventDate}\n`;
        ...
        const message = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: 'text', text: `Domain check failed: ${message}` }],
            isError: true,
        };
    }
});
// ─── Tool: bulk_domain_check ─────────────────────────────────────────────────
server.tool('bulk_domain_check', `Check multiple domains for availability concurrently.
...
Useful for finding available domains across multiple TLDs or name variations.`, {
    ...
    results.sort((a, b) => (domainOrder.get(a.domain) ?? 0) - (domainOrder.get(b.domain) ?? 0));
    const bulkResult = {
        results,
...
// ─── Tool: tld_info ──────────────────────────────────────────────────────────
server.tool('tld_info', `Get information about a TLD (Top-Level Domain) from IANA.
...
Works for gTLDs (.com, .org), new gTLDs (.app, .xyz), and ccTLDs (.uk, .de).`, {
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: 'text', text: `TLD info lookup failed: ${message}` }],
            isError: true,
        };
    }
});
// ─── Tool: whois_raw ─────────────────────────────────────────────────────────
server.tool('whois_raw', `Send a raw WHOIS query to a specific server and return the unprocessed response.
    ...
    try {
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: `## Raw WHOIS Response from ${whoisServer}:${port}\n\n` +
                        '```\n' +
                        rawText +
                        '\n```',
                },
        ...
        const message = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: 'text', text: `Raw WHOIS query failed: ${message}` }],
            isError: true,
        };
    }
});
// ─── Tool: find_whois_server ─────────────────────────────────────────────────
server.tool('find_whois_server', `Discover the WHOIS and RDAP servers for a domain or TLD.
...
Returns both WHOIS and RDAP server information.`, {
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: 'text', text: `Server discovery failed: ${message}` }],
            isError: true,
        };
    }
});
// ─── Tool: rdap_bootstrap_info ───────────────────────────────────────────────
server.tool('rdap_bootstrap_info', `Show the IANA RDAP bootstrap data — lists all TLDs that support RDAP and their service URLs.
Useful for understanding RDAP coverage across TLDs.`, {
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: 'text', text: `RDAP bootstrap fetch failed: ${message}` }],
            isError: true,
        };
    }
});
 ...
 */
async function checkDomain(domain, method, timeout) {
