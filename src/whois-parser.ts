...
import type { WhoisResult, ContactInfo } from './types.js';
...
export function isDomainAvailable(rawText: string): boolean {
...
export function parseWhoisResponse(
  ...
  // Dates
  result.creationDate = normalizeDate(
    getFirst(kvPairs, [
      'Creation Date',
      'Created Date',
      'Created',
  ...
  result.expirationDate = normalizeDate(
    getFirst(kvPairs, [
  ...
  result.updatedDate = normalizeDate(
    getFirst(kvPairs, [
      'Updated Date',
      'Last Updated',
      'Last Modified',
 ...
 */
export function extractReferralServer(rawText: string): string | null {
  const patterns = [
