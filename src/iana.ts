 ...
 */
export function extractSecondLevelTld(domain: string): string | null {
  const parts = domain.toLowerCase().replace(/\.$/, '').split('.');
 ...
 */
export async function queryIanaWhoisServer(
  tld: string,
  timeout = 10000,
): Promise<{ whoisServer: string | null; rawResponse: string }> {
 ...
 */
export async function findWhoisServer(
  domain: string,
  timeout = 10000,
): Promise<string | null> {
 ...
 */
export async function getRdapBootstrap(): Promise<Map<string, string>> {
  const now = Date.now();
 ...
 */
export async function findRdapServer(domain: string): Promise<string | null> {
  try {
 ...
 */
export async function getTldInfo(tld: string): Promise<{
