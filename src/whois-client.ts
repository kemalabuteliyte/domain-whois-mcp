 ...
 */
const QUERY_FORMATS: Record<string, (domain: string) => string> = {
...
export async function whoisLookup(
  ...
  if (!server) {
    ...
    if (!server) {
      throw new Error(
        `No WHOIS server found for "${normalizedDomain}". ` +
  ...
  const result = await queryWithReferrals(
    normalizedDomain,
    server,
    options.port ?? 43,
    timeout,
 ...
 */
export async function whoisRawQuery(
  domain: string,
  server: string,
  port = 43,
  timeout = 15000,
...
export async function whoisCheck(
  domain: string,
  options: WhoisQueryOptions = {},
): Promise<{ available: boolean; result: WhoisResult }> {
 ...
 */
async function queryWithReferrals(
  domain: string,
  server: string,
  port: number,
  timeout: number,
  ...
  if (followReferrals && depth < MAX_REFERRAL_DEPTH) {
    ...
    if (referralServer && referralServer.toLowerCase() !== server.toLowerCase()) {
      try {
        const referralResult = await queryWithReferrals(
          domain,
          referralServer,
          port,
          timeout,
        ...
        if (
          !isDomainAvailable(referralResult.rawText) &&
